...
def test_partseg(model, loader, catdict, num_classes = 50,forpointnet2=False):
    ...
    for batch_id, (points, label, target, norm_plt) in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
        ...
        p, l, t, n = points.data.numpy(), label.data.numpy(), target.data.numpy(), norm_plt.data.numpy()
        s_pred = np.array(seg_pred)
        point_infos = np.concatenate((p, n, l, t, s_pred), dim = 1)
        np.savetxt('../Lzr/experiment/result_'+time+'/'+str(batch_id)+'.txt', point_infos, fmt="%d", delimiter=" ")
