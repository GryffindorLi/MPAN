...
def main(args):
    ...
    file_dir = Path(
        str(experiment_dir) + '/%sPartSeg-' % args.model_name + str(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')))
    ...
    dataloader = torch.utils.data.DataLoader(TRAIN_DATASET, batch_size=args.batchsize, shuffle=True,
    ...
    model = PointNet2PartSeg_msg_one_hot(num_part) if args.model_name == 'pointnet2' else PointNetDenseCls(
    ...
    for epoch in range(init_epoch, args.epoch):
        ...
        print('Epoch %d %s accuracy: %f  Class avg mIOU: %f   Inctance avg mIOU: %f' % (
            epoch, blue('test'), test_metrics['accuracy'], test_metrics['class_avg_iou'],
